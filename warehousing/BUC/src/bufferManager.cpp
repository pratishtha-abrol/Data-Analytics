...
void BufferManager::updatePageName(Page p, string Name)
{
    for(auto it = this->pages.begin(); it != this->pages.end(); ++it)
    {
        if(it->pageName == p.pageName)
 ...
 * @param rowCount 
 */
void BufferManager::writePage(string tableName, int pageIndex, vector<vector<string>> rows, int rowCount)
{
