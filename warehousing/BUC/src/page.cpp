...
Page::Page(string tableName, int pageIndex)
{
    ...
    if (matrixCatalogue.isMatrix(this->tableName))
    {
        ...
        uint maxColCount = table.maxColsPerBlock;
        vector<string> row(maxColCount, "");
        this->rows.assign(maxRowCount, row);
        ifstream fin(pageName, ios::in);
        this->rowCount = table.rowsPerBlockCount[pageIndex];
        this->colCount = table.colsPerBlockCount[pageIndex];
        string number;
        for (uint rowCounter = 0; rowCounter < this->rowCount; rowCounter++)
        {
            for (uint columnCounter = 0; columnCounter < this->colCount; columnCounter++)
            {
                fin >> number;
 ...
 * @return vector<int>
 */
vector<string> Page::getRow(int rowIndex)
{
...
Page::Page(string tableName, int pageIndex, vector<vector<string>> rows, int rowCount)
{
    logger.log("Page::Page");
    this->tableName = tableName;
    this->pageIndex = pageIndex;
    ...
    this->colCount = rows[0].size();
    this->pageName = "../data/temp/" + this->tableName + "_Page" + to_string(pageIndex);
}

void Page::updatePageName(string Name)
{
    this->pageName = Name;
}
